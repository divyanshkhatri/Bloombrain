...
export const events = ({token, page, order}) => {
  ...
  const headerParams = { 
    ...
    return((dispatch) => {
      axios.get(`https://test-ecp.codepark.in/moderate/event/activity/parkpandemic?node=${page}&order=${order}`, config )
           .then((response) => {
              event.push(response.data.activities);
              console.log(response);
              dispatch( { type : 'eventsList', payload: event } )
           })
    })

}

export const neworderevents = ({token, page, order}) => {
...
export const moreEvents = ({token, page, order}) => {
